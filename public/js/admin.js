  ...
  socket.emit('admin_list_messages_by_user', params, messages => {
    ...
    messages.forEach(message => {
      ...
      if (!message.admin_id) {
        ...
        createDiv.innerHTML = `<span style="font-size: 12px">${connection.user.email}</span>`;
        ...
        createDiv.innerHTML = '<span style="font-size: 12px">Atendente</span>';
        createDiv.innerHTML += `<span>${message.text}</span>`;
        createDiv.innerHTML +=
          `<span class="admin_date">
            ${dayjs(message.created_at).format('DD/MM/YYYY HH:mm:ss')}
...
socket.on('admin_receive_message', (data) => {
  const connection = connectionsUsers
    .find(connection => connection.socket_id === data.socket_id);
  ...
  createDiv.innerHTML = `<span style="font-size: 0.825rem">${connection.user.email}</span>`;
